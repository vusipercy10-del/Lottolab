...
function combinations(draw: Draw, size: 2|3) {
  const out: string[] = [];
  for (let i=0;i<draw.length;i++) for (let j=i+1;j<draw.length;j++) {
    if (size===2) out.push(pairKey(draw[i],draw[j]));
    else for (let k=j+1;k<draw.length;k++) out.push(tripKey(draw[i],draw[j],draw[k]));
  }
  return out;
}
...
function analyse(draws: Draw[], game: Game, weights: Record<string,number>): Score[] {
  ...
  draws.forEach((d,idx)=>{
    d.forEach(n=>{freq[n]++;last[n]=idx;ending[n%10]++;});
    combinations(d,2).forEach(k=>pairs.set(k,(pairs.get(k)||0)+1));
    combinations(d,3).forEach(k=>trips.set(k,(trips.get(k)||0)+1));
    if(idx) for(const a of draws[idx-1]) for(const b of d) { const k=`${a}>${b}`; transitions.set(k,(transitions.get(k)||0)+1); }
  });
  const maxF=Math.max(1,...freq), maxE=Math.max(1,...ending), maxP=Math.max(1,...pairs.values(),0), maxT=Math.max(1,...trips.values(),0), maxTr=Math.max(1,...transitions.values(),0);
  const latest=draws[draws.length-1]||[];
  return Array.from({length:game.max},(_,i)=>i+1).map(n=>{
    const pair=Math.max(0,...latest.filter(a=>a!==n).map(a=>pairs.get(pairKey(n,a))||0));
    const trip=Math.max(0,...latest.flatMap(a=>latest.filter(b=>b!==a&&b!==n).map(b=>trips.get(tripKey(n,a,b))||0)),0);
    const markov=Math.max(0,...latest.map(a=>transitions.get(`${a}>${n}`)||0));
    const gap=last[n]<0?draws.length:draws.length-1-last[n];
    const z=draws.flat().filter(x=>zone(x,game.max)===zone(n,game.max)).length;
    const score = (freq[n]/maxF)*weights.frequency + Math.min(1,gap/Math.max(1,draws.length/3))*weights.gap + (pair/maxP)*weights.pair + (trip/maxT)*weights.triplet + (markov/maxTr)*weights.markov + Math.min(1,z/Math.max(1,draws.length*game.pick/5))*weights.zone + (ending[n%10]/maxE)*weights.ending;
    return {n,score,frequency:freq[n],gap,pair,triplet:trip,markov,zone:zone(n,game.max),ending:n%10};
...
export default function ResearchLab() {
  const [gameId,setGameId]=useState('3/15');
  ...
  const draws=useMemo(()=>parse(raw,game),[raw,game]);
  const weights={frequency,gap,pair,triplet,markov,zone:zoneW,ending};
  const scores=useMemo(()=>analyse(draws,game,weights),[draws,game,frequency,gap,pair,triplet,markov,zoneW,ending]);
  const candidate=pickCandidate(scores,game);
  const [saved,setSaved]=useState<Draw[]>([]);
  const [actual,setActual]=useState('');
  const [backtest,setBacktest]=useState<{avg:number;best:number;zero:number;tests:number}|null>(null);
  const [sim,setSim]=useState<{avg:number;p95:number}|null>(null);

  function runBacktest(){
    if(draws.length<Math.max(5,window+1)) return;
    let total=0,best=0,zero=0,tests=0;
    for(let i=window;i<draws.length;i++){
      const train=draws.slice(Math.max(0,i-window),i);
      const s=analyse(train,game,weights); const pred=pickCandidate(s,game); const m=matches(pred,draws[i]);
      total+=m; best=Math.max(best,m); if(!m) zero++; tests++;
    }
    setBacktest({avg:Number((total/tests).toFixed(2)),best,zero,tests});
  }
  function runSimulation(){
    const trials=2000; let total=0; const counts:number[]=[];
    for(let t=0;t<trials;t++){
      const set=new Set<number>(); while(set.size<game.pick) set.add(1+Math.floor(Math.random()*game.max));
      total+=matches(candidate,[...set]); counts.push(matches(candidate,[...set]));
    }
  ...
  return <main style={{minHeight:'100vh',padding:24,fontFamily:'system-ui',background:'#0b1020',color:'#eef2ff'}}>
    <div style={{maxWidth:1250,margin:'0 auto'}}>
      <header style={{display:'flex',justifyContent:'space-between',gap:20,alignItems:'center',marginBottom:24,flexWrap:'wrap'}}>
        <div><div style={{fontSize:13,letterSpacing:2,opacity:.7}}>LOTTOLAB • V13 RESEARCH ENGINE</div><h1 style={{fontSize:34,margin:'6px 0'}}>Strategy Lab + Backtesting</h1><div style={{opacity:.75}}>Data validation, ensemble scoring, walk-forward testing, prediction tracking and Monte Carlo baseline.</div></div>
